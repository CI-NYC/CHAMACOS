...
data <- read_csv("data/rudolph_01b.csv") |>
    ...
    mutate(first_obs = case_when(years_2y_10_5y >= 1 ~ "10_5y", # getting period of "first" observation (where years >= 1), this is time t = 0
                                 years_2y_12y >= 1 ~ "12y",
                                 #years_2y_14y >= 1 ~ "14y",
                                 years_2y_16y >= 1 ~ "16y",
                                 years_2y_18y >= 1 ~ "18y",
  ...
  filter(is.na(first_obs) == FALSE) |> # removing those that are never observed for >= 1 year
...
# fixing outcome for survival
data <- data |>
  mutate(mhtn_12y = ifelse(mhtn_10_5y == 1, 1, mhtn_12y),
         mhtn_14y = ifelse(mhtn_12y == 1, 1, mhtn_14y),
         mhtn_16y = ifelse(mhtn_14y == 1, 1, mhtn_16y), #we can still use 14 year outcome since it uses BP, though it is not complete
...
# for time-varying use LOCF unless baseline, then use modes
  covars_outcome <- covars_outcome |>
    mutate(cham = cham - 1, # making into 0-1
           ...
           age_16y = ifelse(is.na(age_16y), age_14y + 2, age_16y),
           ...
           marstat_16y = ifelse(is.na(marstat_16y),marstat_14y, marstat_16y),
           ...
           marcat_16y = ifelse(is.na(marcat_16y), marcat_14y, marcat_16y),
           ...
           ipovcat_16y = ifelse(is.na(ipovcat_16y), ipovcat_14y, ipovcat_16y),
           ...
           work_cat_16y = ifelse(is.na(work_cat_16y), work_cat_14y, work_cat_16y),
...
covars_outcome <- covars_outcome |>
  ...
  mutate(censor_time_1 = ifelse(is.na(mhtn_time_1), 0, 1),
       censor_time_2 = ifelse(is.na(mhtn_time_2), 0, 1),
       censor_time_3 = ifelse(is.na(mhtn_time_3), 0, 1),
       censor_time_4 = ifelse(is.na(mhtn_time_4), 0, 1),
       censor_time_5 = ifelse(is.na(mhtn_time_5), 0, 1)) |>
...
# LOCF for missing exposures

# potentially look to add 2 each time instead of just once
locf_func_age <- function(data, prefix) {
  cols <- data |> dplyr::select(dplyr::starts_with(prefix))
  
  cols_filled <- t(apply(cols, 1, function(row) {
    filled <- zoo::na.locf(row, na.rm = FALSE)
    filled[is.na(row) & !is.na(filled)] <- filled[is.na(row) & !is.na(filled)] + 2
    filled
  }))
...
locf_func <- function(data, prefix) {
  cols <- data |> select(starts_with(prefix))
  cols_filled <- t(apply(cols, 1, function(row) na.locf(row, na.rm = FALSE)))
  colnames(cols_filled) <- colnames(cols)
  as_tibble(cols_filled)
}
...
covars_outcome <- covars_outcome |>
  ...
  bind_cols(locf_func_age(covars_outcome, "age_time_"))


# once censored or outcome, make any future variables NA
censored_future_vars_NA <- function(df) {
  ...
  for (i in seq_len(nrow(df))) {
    for (t in all_times) {
      c_col <- paste0("censor_time_", t)
      y_col <- paste0("mhtn_time_", t)
      
      c_val <- if (c_col %in% names(df)) df[[c_col]][i] else NA
      y_val <- if (y_col %in% names(df)) df[[y_col]][i] else NA
      
      if ((!is.na(c_val) && c_val == 0) || (!is.na(y_val) && y_val == 1)) {
