...
for (t in 1:5)
{
  ...
  # MULTIPLICATIVE SHIFT: Assume we want to perform 20% shift decreases on glyphosate and glyuat
  #shifted_mult <- as.matrix(mutate(pol, across(c(paste0("gly_kg_2_year_time_", t), paste0("gly_kg_2_year_time_", t)), \(x) x * 0.8)))
  shifted_mult <- as.matrix(mutate(pol, across(c(paste0("gly_kg_2_year_time_", t)), \(x) x * 0.8)))
...
saveRDS(shifted_mult_final, paste0("data/shifted_data_convex_mult_gly_shift.rds"))
saveRDS(prop_in_convex_hull, paste0("data/percent_in_convex_hull_gly_shift.rds"))
saveRDS(R_statistic, paste0("data/R_statistic_gly_shift.rds"))
