#devtools::install_github("nt-williams/lmtp) only works with main branch
library(lmtp)
...
))
{
  ...
  ))
  {
    ...
    data_shifted_mult_all <- readRDS(paste0("data/original_data_time_1_years_obs_", years, ".rds")) |>
      mutate(across(all_of(unlist(A)), ~ . * 0.8)) |>
      mutate(censor_10_5y = 1,
             censor_12y = 1,
             censor_14y = 1,
             censor_16y = 1,
    ...
    data_shifted_add_all <- readRDS(paste0("data/original_data_time_1_years_obs_", years, ".rds")) |>
      ...
      mutate(across(all_of(unlist(A)), ~ . - 10)) |>
      mutate(censor_10_5y = 1,
             censor_12y = 1,
             censor_14y = 1,
             censor_16y = 1,
    ...
    learners <- list("SL.mean", "SL.glm", "SL.xgboost")
    
    run_lmtp <- function(data = data_original, outcome_timepoint = "10_5y", shifted = NULL)
    {
      res <- lmtp_tmle(data, 
                       trt = A,
    ...
    obs <- run_lmtp(data = data_original |>
                      mutate(across(unlist(A), norm01)),
                    outcome_timepoint = time, shifted = NULL)
    saveRDS(obs, here::here(paste0("results/", "mhtn_obs_", time, "_years_observed_",  years, ".rds")))
    
    set.seed(5)
    obs_all <- run_lmtp(outcome_timepoint = time, shifted = NULL)
