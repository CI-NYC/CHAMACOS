...
prop_in_convex_hull_strict <- list()
R_statistic <- list()
...
for (t in 1:5)
{
  ...
  options(JULIA_HOME = "/Users/kararudolph_1/.juliaup/bin")
  ...
  in_hull <- within_error_range
  in_hull_or_extrapolated <- within_error_range | specified_01_range <= 0.05
  
  shifted_final <- ifelse(in_hull_or_extrapolated, shifted_mult, as.matrix(pol))
  shifted_inside <- ifelse(in_hull, shifted_mult, as.matrix(pol))
  
  prop <- mean(in_hull_or_extrapolated[, 1])
  
  prop_in_convex_hull[[t]] <- prop
  
  propstrict <- mean(in_hull[, 1])
  
  prop_in_convex_hull_strict[[t]] <- propstrict
  ...
  shifted_mult_data <- shifted_final |>
   ...
   shifted_mult_data_strict <- shifted_inside  |>
    as.data.frame() |>
    cbind(pol_unnormalized$newid) |>
    rename("newid" = "pol_unnormalized$newid") |>
    relocate(newid, .before = ) # make it the first column
  ...
  shiftedstrict[[t]] <- shifted_mult_data_strict
  observed[[t]] <- obs_data
  R_statistic[[t]] <- R_statistic_df
}

(1 - prop_in_convex_hull_strict[[1]])
mean(R_statistic[[1]]$R>0.1)
mean(R_statistic[[1]]$R_numerator>0.05)

merged_shifted <- reduce(shifted, left_join, by = "newid")
merged_shiftedstrict <- reduce(shiftedstrict, left_join, by = "newid")
...
shifted_mult_final_strict <- covars_outcome |>
  left_join(merged_shiftedstrict, by = c("newid" = "newid"))
...
saveRDS(shifted_mult_final_strict, paste0("data/shifted_data_convex_strict_mult_last_2_shift.rds"))
...
not_in_hull_or_extrapolated_strict_1 <- (1 - prop_in_convex_hull_strict[[1]]) * still_in_study_1
not_in_hull_or_extrapolated_strict_2 <- (1 - prop_in_convex_hull_strict[[2]]) * still_in_study_2
not_in_hull_or_extrapolated_strict_3 <- (1 - prop_in_convex_hull_strict[[3]]) * still_in_study_3
not_in_hull_or_extrapolated_strict_4 <- (1 - prop_in_convex_hull_strict[[4]]) * still_in_study_4
not_in_hull_or_extrapolated_strict_5 <- (1 - prop_in_convex_hull_strict[[5]]) * still_in_study_5
...
(not_in_hull_or_extrapolated_strict_1 + not_in_hull_or_extrapolated_strict_2 + not_in_hull_or_extrapolated_strict_3 + not_in_hull_or_extrapolated_strict_4 + not_in_hull_or_extrapolated_strict_5)/(still_in_study_1 + still_in_study_2 + still_in_study_3 + still_in_study_4 + still_in_study_5)
...
for (t in 1:5)
{
  ...
  # MULTIPLICATIVE SHIFT: Assume we want to perform 20% shift decreases on neo
  #shifted_mult <- as.matrix(mutate(pol, across(c(paste0("gly_kg_2_year_time_", t), paste0("paraq_kg_2_year_time_", t)), \(x) x * 0.8)))
